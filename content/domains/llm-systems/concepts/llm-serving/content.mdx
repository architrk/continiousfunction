...
- TTFT is usually dominated by **prefill**.
- TPOT is usually dominated by **decode** (and KV cache reads).

### Goodput (what production systems optimize)
...
$$\\text{allocated}(T) = \\left\\lceil \\frac{T}{P} \\right\\rceil P,\\qquad
\\text{waste}(T) = \\text{allocated}(T) - T.$$
...
Use the demo to explore TTFT vs TPOT tradeoffs, how batching affects goodput, and why KV cache bandwidth dominates decode latency.
