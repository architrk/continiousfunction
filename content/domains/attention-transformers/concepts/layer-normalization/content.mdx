...
RMSNorm is a popular simplification (used in LLaMA-like models): it drops the mean-centering step and normalizes only by the RMS.
...
$$\\mathrm{RMSNorm}(x) = \\gamma \\odot \\frac{x}{\\mathrm{RMS}(x) + \\epsilon},\\qquad \\mathrm{RMS}(x)=\\sqrt{\\frac{1}{d}\\sum_i x_i^2}.$$
