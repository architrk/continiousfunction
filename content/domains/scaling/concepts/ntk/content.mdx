...
Consider gradient flow on a squared loss over training points $(x_i,y_i)$, with $\mathcal L(\theta) = \tfrac12\sum_i (f_\theta(x_i) - y_i)^2$.

In the infinite-width NTK limit for the standard parameterizations studied in this literature, $\Theta$ becomes deterministic and stays approximately constant during training. The function evolves according to:
