...
The division is elementwise, so Adam acts like a diagonal preconditioner: coordinates with consistently large gradients get smaller effective steps, while coordinates with consistently small gradients get larger relative steps. Bias correction matters most early in training because the moving averages are initialized at zero. The small $\varepsilon$ is not a learning signal; it prevents division by zero and can affect stability when gradients are tiny.
