...
Mixture of Experts (MoE) is the "specialist team" version of a neural network.
...
But it also sells you new problems: tokens get unevenly routed (skew), which creates stragglers and all-to-all communication. In MoE, "sparsity" is as much a **systems** story as a modeling story.
...
$$\\mathrm{MoE}(h)=\\sum_{e\\in S}\\tilde p_e\\,f_e(h),\\qquad
\\tilde p_e = \\frac{p(e\\mid h)}{\\sum_{j\\in S} p(j\\mid h)}.$$

### Load balancing (avoid expert collapse)

Routers tend to collapse onto a small subset of experts unless you regularize. A classic form uses:
...
Use the demo to watch routing decisions, expert load, and how load balancing changes behavior.
