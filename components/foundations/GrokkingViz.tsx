...
const PRESET_MOMENTS = [
  { name: '🚀 Start', value: 0, description: 'Beginning of training' },
  { name: '📝 Memorized', value: 25, description: 'Model has memorized training set' },
  { name: '🤔 Pre-grok', value: 75, description: 'Just before the breakthrough' },
  { name: '💡 Grokking!', value: 82, description: 'The magical moment' },
  { name: '🎓 Generalized', value: 95, description: 'Model has learned the algorithm' },
]
...
const GrokkingGrokViz: React.FC = () => {
  ...
  const checkPrediction = () => {
    ...
    const predProgress = prediction / 100
    const trueGrokPoint = GROK_POINT
    const errorPercent = Math.abs(predProgress - trueGrokPoint) * 100
    const score = Math.max(0, 100 - errorPercent * 2)
    setRevealed(true)
    setAttempts((prev) => prev + 1)
    if (bestScore === null || score > bestScore) {
      setBestScore(score)
    }
    // Animate to reveal the true grok point
    setSlider(Math.round(GROK_POINT * 100))
  }

  // Reset prediction game
  const resetPrediction = () => {
    setPrediction(null)
    setRevealed(false)
    setSlider(Math.floor(Math.random() * 30) + 5) // Random starting point
  }
    ...
    <div className="grok-root">
      ...
      <div
        style={{
          ...
          borderRadius: '10px',
          background: gameMode
            ? 'linear-gradient(135deg, rgba(249, 115, 22, 0.2) 0%, rgba(245, 158, 11, 0.1) 100%)'
            : 'rgba(15, 23, 42, 0.5)',
          border: gameMode ? '2px solid #f97316' : '1px solid rgba(148, 163, 184, 0.2)',
        }}
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: gameMode ? '1rem' : 0 }}>
          ...
          >
            {gameMode ? '🛑 Exit Game' : '🎮 Predict the Grok!'}
          </button>

          {gameMode && bestScore !== null && (
            <div style={{ display: 'flex', gap: '1.5rem', fontSize: '0.9rem', fontWeight: 600, color: '#e5e7eb' }}>
        ...
        {gameMode && (
          <div style={{ marginTop: '0.75rem' }}>
            <p style={{ fontWeight: 500, marginBottom: '0.75rem', color: '#e5e7eb' }}>
              🤔 <strong>Challenge:</strong> At what training step will the model suddenly &quot;grok&quot; (validation loss collapses)?
            </p>

            {!revealed ? (
              <>
                <div style={{ marginBottom: '0.75rem' }}>
                  <label style={{ display: 'block', marginBottom: '0.5rem', color: '#9ca3af', fontSize: '0.85rem' }}>
                    Your prediction: {prediction !== null ? `${prediction}% (≈ 10^${(prediction / 100 * LOG_MAX_STEPS).toFixed(1)} steps)` : 'Click the slider to guess'}
                  </label>
                  <input
                    type="range"
                    min={0}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                <span
                  style={{
                    padding: '0.5rem 1rem',
                    borderRadius: '8px',
                    background: Math.abs((prediction ?? 0) / 100 - GROK_POINT) < 0.1
                      ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)'
                      : Math.abs((prediction ?? 0) / 100 - GROK_POINT) < 0.2
                ...
                >
                  {Math.abs((prediction ?? 0) / 100 - GROK_POINT) < 0.1
                    ? `🎉 Amazing! Score: ${Math.max(0, 100 - Math.abs((prediction ?? 0) / 100 - GROK_POINT) * 200).toFixed(0)}`
                    : Math.abs((prediction ?? 0) / 100 - GROK_POINT) < 0.2
                    ? `👍 Close! Score: ${Math.max(0, 100 - Math.abs((prediction ?? 0) / 100 - GROK_POINT) * 200).toFixed(0)}`
                    : `❌ Off by ${(Math.abs((prediction ?? 0) / 100 - GROK_POINT) * 100).toFixed(0)}%`}
                </span>
                <span style={{ fontSize: '0.85rem', color: '#9ca3af' }}>
                  (True grok point: {GROK_POINT * 100}% ≈ 10^{(GROK_POINT * LOG_MAX_STEPS).toFixed(1)} steps)
                </span>
                <button
                  type="button"
      ...
      <div className="grok-controls">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
          <span style={{ fontSize: '0.85rem', color: '#9ca3af', marginRight: '0.25rem' }}>🎮 Jump to:</span>
          {PRESET_MOMENTS.map((preset) => (
            <button
              key={preset.name}
              type="button"
              onClick={() => {
                setSlider(preset.value)
                setIsPlaying(false)
              }}
              ...
              style={{
                ...
                borderRadius: '6px',
                border: Math.abs(slider - preset.value) < 3
                  ? '2px solid #14b8a6'
                  : '1px solid rgba(148, 163, 184, 0.3)',
                background: Math.abs(slider - preset.value) < 3
                  ? 'rgba(20, 184, 166, 0.2)'
                  : 'rgba(15, 23, 42, 0.8)',
                color: '#e5e7eb',
                cursor: 'pointer',
                fontSize: '0.8rem',
                fontWeight: Math.abs(slider - preset.value) < 3 ? 600 : 400,
              }}
            >
              {preset.name}
            </button>
