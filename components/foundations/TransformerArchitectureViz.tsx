...
const ARCHITECTURE_CHALLENGES: ArchitectureChallenge[] = [
  {
    name: '🎲 The Chatbot',
    scenario: 'You need to build an AI that generates text one token at a time, predicting the next word based on everything before it.',
    hint: 'Think: does this need to see future tokens?',
    answer: 'gpt',
    explanation: 'GPT\'s decoder-only architecture with causal masking is perfect for autoregressive generation. Each token only sees past tokens—exactly what you need for next-token prediction.',
  },
  {
    name: '🎲 The Translator',
    scenario: 'You\'re building a machine translation system that needs to read an entire French sentence before generating the English translation.',
    hint: 'The input and output are different sequences...',
    answer: 'original',
    explanation: 'The original Transformer\'s encoder-decoder design is ideal: the encoder reads the full source sentence, then the decoder generates the target sequence while attending to the encoder outputs via cross-attention.',
  },
  {
    name: '🎲 The Classifier',
    scenario: 'You need to classify whether a movie review is positive or negative. The model should understand the full context in both directions.',
    hint: 'Classification needs to see the whole input...',
    answer: 'bert',
    explanation: 'BERT\'s encoder-only architecture with bidirectional attention lets every token see all other tokens. Perfect for understanding tasks where you need to classify the entire input.',
  },
  {
    name: '🎲 The Summarizer',
    scenario: 'You want one model that can summarize, translate, and answer questions—all using the same "text in, text out" paradigm.',
    hint: 'Unified format for many tasks...',
    answer: 't5',
    explanation: 'T5 treats every NLP task as text-to-text generation. Its encoder-decoder architecture with relative positional encodings handles diverse tasks: "summarize: [article]" → summary, "translate: [text]" → translation.',
  },
  {
    name: '🎲 The Embedder',
    scenario: 'You need to create semantic embeddings of sentences for similarity search. The representation should capture meaning from the entire sentence.',
    hint: 'Sentence embeddings need full context...',
    answer: 'bert',
    explanation: 'BERT\'s bidirectional encoder creates rich representations where each token incorporates information from the entire sequence. Pool these outputs to get sentence embeddings.',
  },
]

function getChallengeFeedback(
  prediction: ArchitectureVariant | null,
  challenge: ArchitectureChallenge
): string {
  if (!prediction) return ''

  const isCorrect = prediction === challenge.answer
  const correctPreset = ARCHITECTURE_PRESETS.find(p => p.id === challenge.answer)
  const predictedPreset = ARCHITECTURE_PRESETS.find(p => p.id === prediction)

  if (isCorrect) {
    return `✅ Correct! ${challenge.explanation}`
  }

  // Wrong answer - explain why the correct answer is better
  return `❌ Not quite. You chose ${predictedPreset?.emoji} ${predictedPreset?.name}, but ${correctPreset?.emoji} ${correctPreset?.name} is the better fit.\n\n${challenge.explanation}`
}
...
const ARCHITECTURE_PRESETS: ArchitecturePreset[] = [
  {
    ...
    name: 'Original',
    emoji: '📖',
    description: 'Vaswani et al. encoder-decoder for translation',
    insight: 'The original Transformer uses full encoder-decoder with cross-attention. Encoder sees all tokens; decoder is causal. 6 layers each, d_model=512.',
  },
  {
    id: 'gpt',
    name: 'GPT',
    emoji: '🔮',
    description: 'Decoder-only for autoregressive generation',
    insight: 'GPT removes the encoder entirely. Decoder-only + causal mask enables next-token prediction. No cross-attention needed—just self-attention throughout.',
  },
  {
    id: 'bert',
    name: 'BERT',
    emoji: '🎭',
    description: 'Encoder-only with bidirectional context',
    insight: 'BERT uses encoder-only with no causal mask—each token sees all others. Trained with masked language modeling (MLM). Great for understanding tasks.',
  },
  {
    id: 't5',
    name: 'T5',
    emoji: '🔄',
    description: 'Text-to-text with relative positions',
    insight: 'T5 returns to encoder-decoder but uses relative positional biases instead of sinusoidal PE. Every NLP task becomes "text in → text out."',
  },
]

// Dynamic insight based on focused component
function getComponentInsight(
  focus: FocusTarget,
  variant: ArchitectureVariant
): { text: string; color: string; emoji: string } {
  const variantPreset = ARCHITECTURE_PRESETS.find(p => p.id === variant)

  if (focus.kind === 'overview') {
    return {
      emoji: variantPreset?.emoji || '🔬',
      color: '#6366f1',
      text: variantPreset?.insight || 'Click on any component to drill down and see detailed dimensions and formulas.',
    }
  }

  // Component-specific insights
  switch (focus.componentId) {
    case 'input-embedding':
      return {
        emoji: '📊',
        color: '#14b8a6',
        text: 'Embedding + positional encoding: Tokens → d_model vectors. Sinusoidal PE lets the model learn relative positions without extra parameters.',
      }
    case 'multihead-self-attn':
      return {
        emoji: '👁️',
        color: '#f59e0b',
        text: 'Multi-head attention: Each head can attend to different patterns. Concat + project gives O(T²·d) complexity. Key architectural bottleneck!',
      }
    case 'add-norm':
      return {
        emoji: '🔗',
        color: '#8b5cf6',
        text: 'Residual + LayerNorm: Skip connections enable gradient flow to early layers. LayerNorm stabilizes activations—crucial for deep networks.',
      }
    case 'feed-forward':
      return {
        emoji: '⚡',
        color: '#22c55e',
        text: 'FFN: Two linear layers with ReLU/GeLU. Expands to d_ff (4×d_model), then projects back. Most parameters live here!',
      }
    case 'encoder-decoder-attn':
      return {
        emoji: '🔀',
        color: '#ef4444',
        text: 'Cross-attention: Decoder queries attend to encoder keys/values. This is how the decoder "reads" the source sequence.',
      }
    case 'output-projection':
      return {
        emoji: '📤',
        color: '#a855f7',
        text: 'Output projection: Linear → softmax over vocabulary. Often shares weights with embedding matrix (tied embeddings).',
      }
    default:
      return {
        emoji: '🔬',
        color: '#6366f1',
        text: 'Click on a component to see its dimensions, formulas, and role in the architecture.',
      }
  }
}
...
export default function TransformerMasterDiagram() {
  ...
  const [selectedChallenge, setSelectedChallenge] = useState<ArchitectureChallenge | null>(null)
  const [prediction, setPrediction] = useState<ArchitectureVariant | null>(null)
  ...
  const startChallenge = useCallback((challenge: ArchitectureChallenge) => {
    ...
    setCountdown(8) // 8 seconds to think and predict
  }, [])

  // Submit prediction
  const submitPrediction = useCallback((variant: ArchitectureVariant) => {
    if (gamePhase !== 'countdown' || !selectedChallenge) return
    setPrediction(variant)
    setGamePhase('revealed')
    // Update score if correct
    if (variant === selectedChallenge.answer && !completedChallenges.has(selectedChallenge.name)) {
      setScore((s) => s + 1)
  ...
  // Dynamic insight based on focus and variant
  const currentInsight = useMemo(() => {
    return getComponentInsight(focus, activeVariant)
  }, [focus, activeVariant])
        ...
        {ARCHITECTURE_PRESETS.map((preset) => (
          <button
            ...
            style={{
              ...
              borderRadius: '999px',
              border: activeVariant === preset.id ? '2px solid #a855f7' : '1px solid rgba(148,163,184,0.4)',
              background: activeVariant === preset.id
                ? 'linear-gradient(135deg, rgba(168,85,247,0.25), rgba(99,102,241,0.15))'
                : 'rgba(15,23,42,0.8)',
              color: activeVariant === preset.id ? '#e5e7eb' : '#9ca3af',
              fontSize: '0.78rem',
              cursor: 'pointer',
            ...
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                <h3 style={{ margin: 0, fontSize: '0.9rem', color: '#e5e7eb' }}>
                  🎯 Architecture Identification Challenge
                </h3>
                <span style={{ fontSize: '0.75rem', color: '#22c55e' }}>
                  Score: {score}/{ARCHITECTURE_CHALLENGES.length}
                </span>
              </div>
              <p style={{ margin: '0 0 0.6rem', fontSize: '0.78rem', color: '#9ca3af' }}>
                Can you match the use case to the right Transformer variant? Select a challenge to test your architectural intuition!
              </p>
            ...
            <>
              ...
              <div style={{
                ...
                <p style={{ margin: 0, fontSize: '0.82rem', color: '#e5e7eb', lineHeight: 1.5 }}>
                  <strong>Scenario:</strong> {selectedChallenge.scenario}
                </p>
              ...
              <p style={{ margin: '0 0 0.5rem', fontSize: '0.78rem', color: '#9ca3af' }}>
                Which architecture is best suited for this task?
              </p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {ARCHITECTURE_PRESETS.map((preset) => (
                  <button
            ...
            <>
              ...
              <div style={{
                ...
                <p style={{ margin: 0, fontSize: '0.82rem', color: '#e5e7eb', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>
                  {getChallengeFeedback(prediction, selectedChallenge)}
                </p>
                {!prediction && (
                  <p style={{ margin: 0, fontSize: '0.82rem', color: '#f59e0b' }}>
                    ⏰ Time's up! The correct answer is {ARCHITECTURE_PRESETS.find(p => p.id === selectedChallenge.answer)?.emoji} {ARCHITECTURE_PRESETS.find(p => p.id === selectedChallenge.answer)?.name}.
