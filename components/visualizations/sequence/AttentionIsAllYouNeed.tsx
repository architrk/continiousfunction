...
// ─────────────────────────────────────────────────────────────
// Gamification: "Architecture Challenge" – predict transformer behavior
// ─────────────────────────────────────────────────────────────
type GamePhase = 'setup' | 'countdown' | 'revealed'
type ArchitecturePrediction = 'A' | 'B' | 'C' | null
...
const ARCHITECTURE_CHALLENGES: ArchitectureChallenge[] = [
  {
    name: '🎲 Scaling Factor',
    question: 'Why does attention divide by √d_k in "softmax(QKᵀ / √d_k)"?',
    optionA: 'To normalize the output to unit variance',
    optionB: 'Dot products grow with dimension, causing vanishing softmax gradients',
    optionC: 'To make attention heads compute different patterns',
    answer: 'B',
    insight: 'Dot products scale as O(√d_k) in magnitude. Without scaling, large d_k causes softmax to concentrate on one key (gradient → 0). The √d_k factor keeps the variance stable regardless of head dimension!'
  },
  {
    name: '🎲 Positional Encoding',
    question: 'Why does the Transformer need positional encoding?',
    optionA: 'Attention is permutation-equivariant - it treats {a,b,c} the same as {c,a,b}',
    optionB: 'To reduce the sequence length during computation',
    optionC: 'To create different attention heads',
    answer: 'A',
    insight: 'Self-attention computes the same outputs regardless of input order (it\'s a set operation!). Positional encoding breaks this symmetry by adding position-dependent signals, letting the model know that "cat sat mat" ≠ "mat cat sat".'
  },
  {
    name: '🎲 Cross-Attention',
    question: 'In encoder-decoder attention, where do Q, K, V come from?',
    optionA: 'All three (Q, K, V) come from the encoder',
    optionB: 'Q from decoder, K and V from encoder',
    optionC: 'Q and K from decoder, V from encoder',
    answer: 'B',
    insight: 'The decoder\'s Q "queries" the encoder\'s K and V. This lets each target position ask: "Which source positions should I attend to?" The answer comes from the encoder\'s learned representations (K, V).'
  },
  {
    name: '🎲 Residual Streams',
    question: 'What\'s the purpose of "Add & Norm" after each sublayer?',
    optionA: 'To reduce the number of parameters',
    optionB: 'To allow gradients to flow directly and stabilize training',
    optionC: 'To compute different attention patterns',
    answer: 'B',
    insight: 'The residual connection (x + Sublayer(x)) creates a direct path for gradients, preventing vanishing gradients in deep networks. LayerNorm stabilizes activations by normalizing across the feature dimension. Together they enable training 100+ layer transformers!'
  }
]
...
export default function TransformerMasterDiagram() {
  ...
  const [prediction, setPrediction] = useState<ArchitecturePrediction>(null)
  const [countdown, setCountdown] = useState<number>(3)
  const [score, setScore] = useState<{ correct: number; total: number }>({ correct: 0, total: 0 })

  const activeChallenge = activeChallengeIdx !== null ? ARCHITECTURE_CHALLENGES[activeChallengeIdx] : null
  ...
  return (
    <div ref={containerRef} style={styles.root}>
      ...
      {/* ─────────────────────────────────────────────────────────────
          GAME PANEL: Architecture Challenge
          ───────────────────────────────────────────────────────────── */}
      <div style={{
        marginTop: 16,
        padding: 16,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h3 style={{ fontSize: '1rem', margin: 0 }}>🎮 Architecture Challenge</h3>
        ...
        {gamePhase === 'revealed' && activeChallenge && (
          <div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#a78bfa', marginBottom: 4 }}>
                💡 Insight
              </div>
              <p style={{ fontSize: '0.85rem', margin: 0, color: '#e5e7eb' }}>
