...
type MambaPrediction = 'A' | 'B' | 'C' | null
...
const MAMBA_CHALLENGES: MambaChallenge[] = [
  {
    name: '🎲 Large Δ Effect',
    question: 'When Δ is LARGE for a token, what happens to the hidden state?',
    optionA: 'Mostly copies previous state',
    optionB: 'Focuses on current token',
    optionC: 'Resets to zero',
    answer: 'B',
    insight: 'h_t = (1-Δ)h_{t-1} + Δx_t. When Δ≈1, the update is mostly the current input x_t. This lets Mamba "write" important tokens into memory while ignoring noise (Δ≈0).'
  },
  {
    name: '🎲 LTI Limitation',
    question: 'Why does LTI (fixed A,B,C) struggle with selective copying?',
    optionA: 'Too few parameters',
    optionB: 'Can\'t ignore noise',
    optionC: 'Runs too slowly',
    answer: 'B',
    insight: 'LTI models have FIXED dynamics regardless of input content. They can\'t "decide" to ignore noise tokens vs remember passwords—every token gets the same treatment, causing memory pollution.'
  },
  {
    name: '🎲 Password Detection',
    question: 'How does Mamba decide which tokens are the password?',
    optionA: 'Δ is learned from input',
    optionB: 'Uses hardcoded rules',
    optionC: 'Random selection',
    answer: 'A',
    insight: 'Mamba computes Δ_t = f(x_t) where f is a learned function! During training, the model learns to produce high Δ for relevant tokens (passwords) and low Δ for noise. This is "selectivity".'
  },
  {
    name: '🎲 Parallel Scan',
    question: 'How can Mamba compute the recurrence h_t = f(h_{t-1}, x_t) efficiently?',
    optionA: 'Sequential only, O(T)',
    optionB: 'Parallel scan, O(log T)',
    optionC: 'No recurrence, pure attention',
    answer: 'B',
    insight: 'The associativity of the recurrence allows a parallel scan: combine pairs → pairs of pairs → etc. This reduces effective sequence depth from O(T) to O(log T), enabling hardware-efficient training!'
  }
]

function getMambaFeedback(predicted: MambaPrediction, challenge: MambaChallenge): string {
  if (!predicted) return ''
...
export default function MambaSelectiveStateSpaceDemo() {
  ...
  const [activeChallenge, setActiveChallenge] = useState<MambaChallenge | null>(null)
  const [prediction, setPrediction] = useState<MambaPrediction>(null)
  const [countdown, setCountdown] = useState<number>(3)
  const [score, setScore] = useState<{ correct: number; total: number }>({ correct: 0, total: 0 })

  function startChallenge(challenge: MambaChallenge) {
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
          <span style={{ fontSize: '0.9rem', fontWeight: 600, color: '#f9fafb' }}>🎮 Selectivity Challenge</span>
          {score.total > 0 && (
            <span style={{ fontSize: '0.8rem', color: '#14b8a6', fontWeight: 500 }}>
        ...
        {!activeChallenge ? (
          <div>
            <p style={{ fontSize: '0.85rem', color: '#9ca3af', margin: '0 0 0.6rem' }}>Test your understanding of Mamba:</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {MAMBA_CHALLENGES.map((ch, idx) => (
                <button
                  key={idx}
          ...
          <div>
            <p style={{ fontSize: '0.9rem', color: '#f9fafb', margin: '0 0 0.7rem', lineHeight: 1.4 }}>{activeChallenge.question}</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '1rem' }}>
            <span style={{ fontSize: '2.2rem', fontWeight: 700, color: '#f59e0b' }}>{countdown}</span>
            <span style={{ fontSize: '0.8rem', color: '#9ca3af' }}>Revealing...</span>
          </div>
        ) : (
          <div style={{ padding: '0.5rem 0' }}>
            <p style={{ fontSize: '0.88rem', lineHeight: 1.5, color: '#e5e7eb', margin: '0 0 0.7rem' }}>
              {getMambaFeedback(prediction, activeChallenge)}
            </p>
            <button
              onClick={resetGame}
              style={{
