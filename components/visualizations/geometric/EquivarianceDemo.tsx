...
type SymmetryPrediction = 'A' | 'B' | null

interface SymmetryChallenge {
  name: string
  question: string
  // Describes what we're comparing
  optionA: string
  optionB: string
  answer: 'A' | 'B'
  insight: string
...
const SYMMETRY_CHALLENGES: SymmetryChallenge[] = [
  {
    name: '🎲 Rotate Classification',
    question: 'You rotate the input arrow 180°. What happens to the classification output?',
    optionA: 'No change (still "arrow")',
    optionB: 'Changes to a different class',
    answer: 'A',
    insight: 'classification is INVARIANT—the class label stays "arrow" regardless of rotation. f(g·x) = f(x)',
  },
  {
    name: '🎲 Translate Detection',
    question: 'You translate the input 50px right. What happens to the detection bounding box?',
    optionA: 'Box stays centered',
    optionB: 'Box moves 50px right',
    answer: 'B',
    insight: 'detection is EQUIVARIANT—the bounding box moves with the input. f(g·x) = g·f(x)',
  },
  {
    name: '🎲 Segmentation Task',
    question: 'For semantic segmentation (labeling each pixel), do we want invariance or equivariance?',
    optionA: 'Invariance',
    optionB: 'Equivariance',
    answer: 'B',
    insight: 'segmentation needs EQUIVARIANCE—if input shifts, pixel labels must shift too! Spatial info is essential',
  },
  {
    name: '🎲 Flip Detection',
    question: 'You flip the input horizontally. In an equivariant network, what happens to the box?',
    optionA: 'Box stays in same position',
    optionB: 'Box flips horizontally',
    answer: 'B',
    insight: 'equivariant networks preserve the transformation: input flips → output flips. f(g·x) = g·f(x)',
  },
]

function getSymmetryFeedback(
  predicted: SymmetryPrediction,
...
export default function EquivarianceInvarianceDemo() {
  ...
  const [activeChallenge, setActiveChallenge] = useState<SymmetryChallenge | null>(null)
  const [prediction, setPrediction] = useState<SymmetryPrediction>(null)
  const [countdown, setCountdown] = useState(3)
  const [gameScore, setGameScore] = useState({ correct: 0, total: 0 })

  // Game control functions
  function startChallenge(challenge: SymmetryChallenge) {
    setActiveChallenge(challenge)
    setPrediction(null)
    setGamePhase('setup')
  }

  function submitPrediction(pred: SymmetryPrediction) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!activeChallenge ? (
          <div>
            <p style={{ color: COLORS.subtle, fontSize: '0.8rem', marginBottom: '0.5rem' }}>
              Test your understanding of equivariance vs invariance:
            </p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
              {SYMMETRY_CHALLENGES.map((ch) => (
                <button
                  key={ch.name}
          ...
          <div>
            <p style={{ color: COLORS.text, fontSize: '0.85rem', marginBottom: '0.6rem' }}>
          ...
          <div style={{ textAlign: 'center', padding: '1rem 0' }}>
            ...
            <p style={{ color: COLORS.subtle, fontSize: '0.8rem', marginTop: '0.3rem' }}>
          ...
          <div>
            ...
            >
              {getSymmetryFeedback(prediction, activeChallenge)}
            </p>
            <button
              onClick={resetGame}
              style={{
