...
import { softmax } from '../lib/mathObjects'
...
* Uses your existing Next.js/TypeScript setup and lives happily alongside the other `components/*` utilities 
* Shows a sequence like `"Harry Potter ... Harry ?"` by default, with tokens laid out along a timeline.
* Draws **two attention patterns as connection diagrams**:

  * Teal arcs (previous-token head) from each position (t) back to (t-1).
  * Orange arcs (induction head) from the second `[A]` back to earlier `[A]`s, with line thickness proportional to attention weight.
* Lets you **step through the circuit**:

  1. *Match earlier [A]* – induction head finds an earlier `"Harry"`.
  2. *Follow to [B]* – previous-token head highlights the `"Potter"` that followed that earlier `"Harry"`.
  3. *Predict [B]* – shows the pattern-completion story “I saw [Harry][Potter] before. I see [Harry] now. Predict [Potter].”
* Uses **line thickness + a dashed animation** to represent attention strength, with smooth transitions between steps.
* Is **interactive**: type any repeated pattern like `"Alice Bob ... Alice ?"` and the circuit recomputes live.
