Here’s a self‑contained Next.js client component using D3 to show:

A teal “3D” (2D contour) loss valley with an orange trajectory.

A sharpness plot with λₘₐₓ(t) and the red stability edge 2/η.

Animation + learning rate slider with three regimes (low / edge / high). 

attachments-bundle

You’ll need d3-scale and d3-shape installed:

bash
Copy code
npm install d3 d3-scale d3-shape
# or
yarn add d3 d3-scale d3-shape


Then create components/EdgeOfStabilityExplorer.tsx:
...
You can drop this into your existing card interactive-card layout alongside components like GradientDescentPlayground and PhasePortrait2D, and it should visually match your explorable-optimization style (orange trajectory, teal landscape, red threshold, dark background).
